...
class TestAccountResetPwd(unittest.TestCase):
    ...
    def test_restpwd(self):
        """ Test api restpwd process """
        header = {'accept-language': 'zh-CN,zh;q=0.8'}
        print "获取验证码token"
        # /api/v1.0/account     /account/v1
        #r = requests.post('http://10.100.250.164:7130/api/v1.0/account/get_captcha_token', headers=header)
        r = requests.post('http://10.100.250.164:7130/account/v1/get_captcha_token', headers=header)
        print 'http://10.100.250.164:7130/account/v1/get_captcha_token'
        print r, r.status_code, r.json()["captcha_token"], r.json()["message"], r.json()["code"]
        print r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["message"], "OK")   #中文显示  操作成功   英文显示  OK
        self.assertEqual(r.json()["code"], 0)
        gl.captcha_token = r.json()["captcha_token"]
        self.assertIsNotNone(gl.captcha_token)

        print "获取验证码"
        r = requests.get('http://10.100.250.164:7130/api/v1.0/account/get_captcha_image' + '?captcha_token=' + gl.captcha_token,
                         headers=header)
        ...
        print "传入参数：" + d
        r = requests.post('http://10.100.250.164:7130/api/v1.0/account/send_verify_code', data=d,header = header)
        print r, "返回值：" + r.text
        self.assertEqual(r.status_code, 200)
        ...
        print "传入参数：" + d
        r = requests.post('http://10.100.250.164:7130/api/v1.0/account/check_verify_code', data=d,header = header)
        print r, "返回值：" + r.text
        self.assertEqual(r.status_code, 200)
        ...
        print "传入参数：" + d
        r = requests.post('http://10.100.250.164:7130/api/v1.0/account/reset_password', data=d,header = header)
        print r, "返回值：" + r.text
        self.assertEqual(r.status_code, 200)
