...
class TestApprovalsProcess(unittest.TestCase):
    ...
    def test_001_get_approvaling_fields_by_approval_name(self):
        ...
        print "请求参数approval_name：" + approval_name + "，id=AQACd5VVkc7RCAAAyQjQcu1N7hTnSwAA"
        # 此处的:object-id为审批列表中的某条data 的id
        r = requests.get(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/fields/" + approval_name + "/AQACd5VVkc7RCAAAyQjQcu1N7hTnSwAA",
    ...
    def test_002_get_can_create_approvals_template(self):
        ...
        approval_name = gl.object_approvals_template_name
        data = {"name": "shangjitest123", "AccountID": "AQACd5VVkc7RCAAAQUXFZjZJ7hToSgAA",
        ...
        print approval_name
        r = requests.post(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/get-available-templates/" + approval_name,
    ...
    def test_003_get_can_update_approval_template(self):
        ...
        print "请求参数：" + json.dumps(data)
        print approval_name
        print gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/get-available-templates/" + approval_name + "/" + gl.object_date_id
        r = requests.put(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/get-available-templates/" + approval_name + "/" + gl.object_date_id,
            data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_004_01_create_an_approval(self):
        ...
        data = {"objects": [
            {"record_type": "main", "name": "fdfdsgdfgdfgdf", "AccountID": "3c5938c9917ca43b73cad4f5453623b3",
             "Amount": {"symbol": "CNY", "value": 10000000}, "CloseDate": "2017-10-21T01:06:07Z", "Stage1": "初步意向",
             "Probability1": "20%", "Stage2": "14天（30天）保持跟进", "Probability2": "20%", "Phone": None,
             "IndustryCategory": None, "Industry": None, "FacilitatorDiscount": 0, "OtherService": None,
             "AffiliatedDepartment": None, "Description": None, "FacilitatorType": None, "Competitor": None,
             "CompetitorPrice": {"symbol": None, "value": None}, "Disagreements": None, "Demands": None,
             "Excitements": None, "FacilitatorBusiness": None, "FacilitatorDoubts": None, "LosingStage": None,
             "LosingReason": None, "LosingDescription": None, "StageUpdatedAt": "2017-10-21T01:06:07Z", "Test": None,
             "Path": None}]}
        print "请求参数data1：" + json.dumps(data)
        r = requests.post(gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/" + gl.meta_name,
                          data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
        ...
        data = {"Name": "审批测试sgk" + str(time.strftime('%Y/%m/%d%H%M%S', time.localtime(time.time()))),
                ...
                "Record": {"name": "商机审批10.11", "AccountID": "AQACd5VVkc7RCAAAQUXFZjZJ7hToSgAA",
                ...
                "PointVoters": [{"PointId": 1, "UserId": "AQAGifYUnayBDAAAn91dvYl07BQoAAAA"},
                                {"PointId": 3, "UserId": "AQACd5VVkc7RCAAAucWwesDL7BR7QAAA"}], "GroupVoters": [],
        ...
        r = requests.post(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/template/" + template_id + "/run",
            data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_004_02_create_an_approval(self):
        ...
        data = {"objects": [{"name": "商机autotest1012160510", "AccountID": "AQACd5VVkc7RCAAAQUXFZjZJ7hToSgAA",
    ...
    def test_005_get_approvals_list(self):
        ...
        # listtype=Vote&orderfield=created_at&odtype=Desc&previous=0&perpage=20
        r = requests.get(
            gl.url_online + "/approval/api/v1.0/meiqia/service/approvals/run?listtype=Vote&orderfield=created_at&odtype=Desc&previous=0&perpage=20",
            headers=headers, verify=False)
        print gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run?listtype=" + listtype + "&orderfield=created_at&odtype=Desc&previous=0&perpage=20"
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
    ...
    def test_006_get_point_approval_by_run_id(self):
        ...
        r = requests.get(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/" + runid,
            headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_007_fill_fields_by_point_id(self):
        ...
        r = requests.put(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/" + runid + "/fill-fields/" + pointid,
            data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_008_01_vote_point_approval_by_run_id(self):
        ...
        r = requests.post(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/" + runid + "/vote",
            data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_008_02_vote_point_approval_by_run_id(self):
        ...
        r = requests.put(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/" + runid + "/fill-fields/2",
            data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
    ...
    def test_010_forward_point_approval_by_run_id(self):
        ...
        print "请求参数run-id：" + runid + "，data：" + json.dumps(data)
        r = requests.post(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/" + runid + "/forward",
            data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_011_add_comments_to_point_approval(self):
        ...
        r = requests.post(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/" + runid + "/comments",
            data=json.dumps(data), headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_013_get_approvals_objects_list_count(self):
        ...
        r = requests.get(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/list-counts",
            headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_014_close_point_approval_by_run_id(self):
        ...
        r = requests.put(
            gl.url_online + "/approval/api/"+gl.api_version+"/" + gl.online_tenant_name + "/service/approvals/run/" + runid + "/close",
            headers=headers, verify=False)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
        # self.assertIsNotNone(result["body"])   #关闭审批之后，返回body 为 null
