...
class TestActions(unittest.TestCase):
    ...
    def test_1_get_all_actions(self):
        """GET /api/gl.api_version/meiqia/actions
        ...
        r = requests.get(gl.url + ":7010/api/" + gl.api_version + "/" + gl.tenant_name + "/actions",
                         headers=headers)
        print gl.url + ":7010/api/" + gl.api_version + "/" + gl.tenant_name + "/actions"
        print r, r.text
    ...
    def test_2_get_meta_actions(self):
        ...
        r = requests.get(
            gl.url + ":7010/api/" + gl.api_version + "/" + gl.tenant_name + "/" + gl.meta_name + "/actions",
            headers=headers)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_3_run_point_meta_action_merge_data_pre(self):
        ...
        r = requests.put(
            gl.url + ":7010/api/" + gl.api_version + "/" + gl.tenant_name + "/" + gl.meta_name + "/actions/run/" + gl.action_merge_data_pre_name,
    ...
    def test_4_run_point_meta_action_merge_data_commit(self):
        ...
        data = {
            "datas": {
                gl.meta_name: [
                    {
                        "Phone": "+86 13111112222",
                        "name": "tdstdd111me"
                    }
                ]
            },
            "optional": {
                "merge_ids": ["AQACk5HxFIZiAwAAPQixhaBg8RQwAQAA", "AQACk5HxFIZiAwAAum83cU0M8RQqAAAA"],
                "delete_ids": []
            }
        }
        ...
        print "请求参数：" + json.dumps(data)
        r = requests.put(
            gl.url + ":7010/api/" + gl.api_version + "/" + gl.tenant_name + "/" + gl.meta_name + "/actions/run/" + gl.action_merge_data_commit_name,
            data=json.dumps(data), headers=headers)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_5_run_point_meta_action_leads_to_account_and_contact_pre(self):
        ...
        data = {
            "datas": {
                "Leads": [
                    {
                        "id": "AQACk5HxFIb0DAAAnJgTmhxd9BTSGQAA"
                    }
                ]
            },
            "optional": {
        ...
        print "请求参数：" + json.dumps(data)
        r = requests.put(
            gl.url + ":7010/api/" + gl.api_version + "/" + gl.tenant_name + "/Leads/actions/run/" + gl.action_leads_to_account_and_contact_pre_name,
            data=json.dumps(data), headers=headers)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
    ...
    def test_6_run_point_meta_action_leads_to_account_and_contact_commit(self):
        ...
        print "请求参数：" + json.dumps(data)
        r = requests.put(
            gl.url + ":7010/api/" + gl.api_version + "/" + gl.tenant_name + "/Leads/actions/run/" + gl.action_leads_to_account_and_contact_commit_name,
            data=json.dumps(data), headers=headers)
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
