...
class TestMPayAll(unittest.TestCase):
    ...
    def test_001_create_customer(self):
        ...
        data = {
            "name": "liubeiqwe",
            "phone": "+86 17888888812",
            "email": "liubeiqwe@qq.com",
            "weixin": "liubeiqwe",
            "type": "CRM_TENANT"
        }
        print "请求参数是：" + json.dumps(data)
        r = requests.post(gl.url_online + ":7145/api/" + gl.api_version + "/mpay/customer", data=json.dumps(data),
                          headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/customer"
        print r, r.text
        ...
        data = {
            "name": "guanyu",
            "phone": "+86 17888888802",
            "email": "guanyu123@qq.com",
            "weixin": "guanyu123",
            "type": "CRM_TENANT_ACCOUNT",
            "external_id": str(time.strftime("%Y%m%d%H%M%S", time.localtime()))    #str(time.strftime("%Y%m%d%H%M%S", time.localtime()))
        }
        print "请求参数是：" + json.dumps(data)
        r = requests.post(gl.url_online + ":7145/api/" + gl.api_version + "/mpay/customer", data=json.dumps(data),
                          headers=headers)
    ...
    def test_002_update_customer_info_by_id(self):
        ...
        print "请求参数是：" + json.dumps(data)
        r = requests.put(gl.url_online + ":7145/api/" + gl.api_version + "/mpay/customer/" + gl.mpay_customer_id_online,
                         data=json.dumps(data), headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/customer/" + gl.mpay_customer_id_online
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
    ...
    def test_003_query_customer_info(self):
        ...
        print "请求参数是：" + json.dumps(data)
        r = requests.get(
            gl.url_online + ":7145/api/" + gl.api_version + "/mpay/customer/query?param=" + urllib.quote(json.dumps(data)),
            headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/customer/query?param=" + urllib.quote(json.dumps(data))
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
    ...
    def test_004_create_account(self):
        ...
        print "请求参数是：" + json.dumps(data)
        r = requests.post(
            gl.url_online + ":7145/api/" + gl.api_version + "/mpay/account",
            data=json.dumps(data), headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/account"
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
    ...
    def test_005_update_account_info_by_id(self):
        ...
        print "请求参数是：" + json.dumps(data)
        r = requests.put(
            gl.url_online + ":7145/api/" + gl.api_version + "/mpay/account/" + gl.mpay_account_id_shoukuan,
            data=json.dumps(data), headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/account/" + gl.mpay_account_id_shoukuan
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
    ...
    def test_006_query_account_info(self):
        ...
        print "请求参数是：" + json.dumps(data)
        r = requests.get(
            gl.url_online + ":7145/api/" + gl.api_version + "/mpay/account/query?param=" + urllib.quote(json.dumps(data)),
            headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/account/query?param=" + urllib.quote(json.dumps(data))
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
    ...
    def test_007_create_bill(self):
        ...
        data = {
            "channel": "MQ_ALI_QRCODE",  #目前支持 阿里：MQ_ALI_QRCODE，微信：MQ_NATIVE，余额消费：MQ_BALANCE
            "amount": 1,
            "currency": "CNY",
            "title": "测试 Bill Pay",
            "timeout":600,
            ...
            "type":"PAY",
            "to_account_id":gl.mpay_account_id_shoukuan_online,   #收款方必须是当前租户，而且关联账户类型必须是美洽（MQ）
            "from_customer_id":gl.mpay_customer_id_online,        #此处传的是租户下CRM_TENANT_ACCOUNT类型的客户id
            "path":gl.mpay_bill_path
        }
        print "请求参数是：" + json.dumps(data)
        r = requests.post(
            gl.url_online + ":7145/api/" + gl.api_version + "/mpay/bill",
            data=json.dumps(data),headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/bill"
        print r, r.text
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()["code"], 0)
        result = json.loads(r.text)
    ...
    def test_008_query_bill_info(self):
        ...
        data = {
            ...
            "type": "PAY",                           #账单类型
            "to_account_id": gl.mpay_account_id_shoukuan_online,     #收款账户ID
            "from_account_id": gl.mpay_customer_id_online,   #付款账户ID
        }
        data = {
            "id":gl.mpay_bill_id,
            "type": "PAY",
        ...
        print "请求参数是：" + json.dumps(data)
        r = requests.get(
            gl.url_online + ":7145/api/" + gl.api_version + "/mpay/bill/query?param="+urllib.quote(json.dumps(data)),
            data=json.dumps(data), headers=headers)
        print gl.url_online + ":7145/api/" + gl.api_version + "/mpay/bill"
        print r, r.text
        self.assertEqual(r.status_code, 200)
